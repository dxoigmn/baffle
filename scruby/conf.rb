...
def initialize()
  ...
  # Net::Pcap:lookupdev(), except on Windows where it will be the
  # second one.
  if $IS_WINDOWS
...
def to_s
  ...
  out += "iface (default interface): #{iface}\n"
  out += "gateway_hwaddr (gateway Ethernet address): #{@gateway_hwaddr}\n"
  out += "promisc (promiscuous mode): #{@promisc}\n"
